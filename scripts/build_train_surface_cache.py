...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help=(
            "optional expected digest for train.json; defaults to the "
            "currently generated split file"
        ),
    )
    parser.add_argument(
        "--expected-split-manifest-sha256",
        help=(
            "optional expected digest for the split manifest; defaults to "
            "the manifest beside train.json"
        ),
    )
    parser.add_argument(
        "--progress-every",
...
def main() -> int:
    ...
    expected_train_split_sha256 = (
        ...
        or _sha256(arguments.train_split)
    )
    expected_split_manifest_sha256 = (
        arguments.expected_split_manifest_sha256
        or _sha256(arguments.train_split.with_name("manifest.json"))
    )
