...
def _validated_config(config: Mapping[str, object]) -> dict[str, object]:
    ...
    _keys(
        ...
        optional=optional_verification | optional_augmentation,
    )
    ...
    if profile not in ("code_b5dc8be", "paper"):
        raise ValueError("loss_profile must be 'code_b5dc8be' or 'paper'")
    ...
    if profile == "paper":
        ...
        raise ValueError(
            "paper_alpha must be omitted for code_b5dc8be loss"
        )
    return {
...
def _validate_effective_noise_scales(
    ...
    *,
    ...
    """Fail closed when a teacher falls outside the estimator's range."""
    ...
    if float(values.min()) < float(minimum) or float(values.max()) > float(
        maximum
    ):
        raise RuntimeError(
            "effective noise-scale teacher is outside the configured "
            f"conditioning range [{float(minimum)}, {float(maximum)}]; "
            "refusing to clip"
        )
    return values
...
def _source_paths(cache_root: Path) -> dict[str, Path]:
